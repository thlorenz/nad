...
function adaptBindingGyp(projectDir, nodeDir, binding_gyp) {
  ...
  binding_gyp.targets = binding_gyp.targets.filter(function (t) {
    if (!t.sources) {
      log.info('nad', 'Skipping target with no sources:', t.target_name);
      return false;
    }
    return true;
  });
...
function isNodeOrIojs(x) { 
  return x.target_name === 'node' ||
         x.target_name === 'iojs' ||
         x.target_name == '<(node_core_target_name)';
}

function getNodeTarget(gyp) {
  if (!gyp.targets) throw new Error('node.gyp is missing "targets"');
  var nodeTarget = gyp.targets.filter(isNodeOrIojs)[0];
  if (!nodeTarget) throw new Error('node.gyp is missing "node" or "iojs" target');
  return nodeTarget;
}
