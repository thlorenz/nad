...
function rewriteNodeGyp(node_gyp, binding_gyp) {
 ...
 * @return {Object} the `node.gyp` content including the necessary changes to make the addon part of the build
 */
function adaptNodeGyp(projectDir, nodeDir, binding_gyp, node_gyp) {
  adaptBindingGyp(projectDir, nodeDir, binding_gyp);
  rewriteNodeGyp(node_gyp, binding_gyp);
