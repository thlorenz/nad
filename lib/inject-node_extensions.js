 ...
 *
 * This is only necessary for Node.js `< v0.11` since `node_extensions.h` and `node_extensions.cc` 
 * drastically changed during `0.11` development until they completely disappeared around `v0.11.12`.
 * Starting with `v0.11.13` an addon module can be loaded dynamically even if it is not registered in the source code.
 *
 *
 * #### Example
 *
...
function injectNodeExtensions(node_extensions_h_file, node_extensions_cc_file, extensions, cb) {
  ...
  function oninjectedH(err) {
    ...
    fs.readFile(node_extensions_cc_file, 'utf8', function (err, src) {
      if (err) return cb(err);
